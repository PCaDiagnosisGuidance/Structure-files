function [AllValuesperMRI, methodsperMRI]=findDataPerMRI()
...
AllValues=[ValuePSA, ValueMRI, ValueBIOPT, ValueECHO];
...
AllValuesperMRI=zeros(maximumID, size(AllValues2, 2));
    ...
    if sum(patientDatesMRI(i, :))>0 %remove all patient without an MRI measurement
        ...
        for j=1:size(AllMethods2, 2)
            if (AllDates(i, j)<=maxDates) && (AllDates(i, j)>0) && (AllDates(i, j)~=90) && (AllDates(i, j)>minDates);
                ...
                AllValuesperMRI(i, j)=AllValues2(i, j);
