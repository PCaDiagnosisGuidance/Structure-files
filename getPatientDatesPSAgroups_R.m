...
%   for i=1:length(techniques)
    ...
    % if it's empty then no data will be added to the groups.
    ...
    if PSA.psa(DateNrFirstPSA,1)<4
        ...
        %make a matrix of dates for PCa (taking the closing date of the DBC
        DateNrDBC=find(PCa.ID==i);
        datesPCa=PCa.date(DateNrDBC)';
    ...
    elseif PSA.psa(DateNrFirstPSA,1)>=10
        ...
        ValuePCa3(i, 1:length(datesPCa))=PCa.PCa(DateNrDBC)';
    end
end
